...
def status_handler(mode, status, ip):
    ...
    display.set_pen(WHITE)
    display.text("Connecting to wifi...", 0,0,WIDTH,3)
    if status is not None:
        if status:
            display.clear()
            display.text("Wifi Connected!", 0,0,WIDTH,3)
            print('Wifi connection successful!')
        else:
            display.set_pen(RED)
            display.clear()
            display.set_pen(WHITE)
            display.text("Wifi Failed", 0,0, WIDTH, 3)
...
button_x.irq(trigger=machine.Pin.IRQ_FALLING, handler=dark_mode)
...
try:
    ...
    display.clear()
    display.set_pen(WHITE)
    display.text("Wifi connection failed! {e}", 0,0, WIDTH, 3)
